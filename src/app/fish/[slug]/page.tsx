...
import { fish, getFish, getFishContent, isBossCreature } from "@/lib/content";
import { createMetadata } from "@/seo/metadata";
import { getFishTdk } from "@/seo/tdk";

export const dynamicParams = false;
export function generateStaticParams() { return fish.map(({ slug }) => ({ slug })); }
...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) { const { slug } = await params; const entry = getFish(slug); if (entry && isBossCreature(entry)) permanentRedirect(`/bosses/${slug}/`); return <FishDetailPage slug={slug} />; }
